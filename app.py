...
with st.sidebar:
    ...
    cities = st.multiselect(
        "City", sorted(df_all.city.unique()), default=sorted(df_all.city.unique())
    )
    ...
    pred_snr = st.selectbox(
        "Doctor seniority", ["Junior", "Mid", "Senior", "Consultant"], key="ps"
    )
...
st.markdown(
    f"<span style='color:var(--muted);font-family:DM Sans'>Showing <b>{len(df):,}</b> appointments across <b>{df.city.nunique()}</b> cities · <b>{df.clinic_name.nunique()}</b> clinics · <b>{df.doctor_name.nunique()}</b> doctors</span>",
    unsafe_allow_html=True,
)
...
def kpi(col, label, val, delta=None, fmt=None):
    if fmt == "money":
        ...
        display = f"{val:,}"
    dhtml = (
        f"<div class='metric-delta' style='color:{'#ef4444' if delta and delta < 0 else '#0d9488'}'>{delta:+.1f}% vs avg</div>"
...
with c1:
    ...
    fig = px.bar(
        ...
        orientation="h",
        color="fee_charged",
        color_continuous_scale=["#ccfbf1", "#0d9488", "#134e4a"],
        labels={"fee_charged": "Revenue (₨)", "city": "City"},
...
st.markdown(
    "<div class='section-head'>Department Performance</div>", unsafe_allow_html=True
)
dept_stats = (
...
with d1:
    fig3 = px.bar(
        dept_stats.sort_values("no_show_rate", ascending=False),
        x="department",
        y="no_show_rate",
        color="no_show_rate",
        color_continuous_scale=["#ccfbf1", "#f59e0b", "#ef4444"],
...
with d2:
    fig4 = px.bar(
        dept_stats.sort_values("revenue", ascending=False),
        x="department",
        y="revenue",
        color="department",
...
st.markdown(
    "<div class='section-head'>Doctor No-Show Rates (Top 15 Worst)</div>",
    unsafe_allow_html=True,
)
doc_ns = (
...
doc_ns = (
    doc_ns[doc_ns.total >= 30].sort_values("no_show_rate", ascending=False).head(15)
)
...
fig5 = px.bar(
    ...
    orientation="h",
    color="department",
    title="Doctors with Highest No-Show Rates",
...
with t2:
    heat = (
        df.groupby(["day_of_week", "appt_hour"])
        .agg(no_show_rate=("status", lambda x: (x == "no_show").mean() * 100))
        .reset_index()
    )
    heat_pivot = heat.pivot(
        index="day_of_week", columns="appt_hour", values="no_show_rate"
    )
    heat_pivot = heat_pivot.reindex(dow_order)
    fig7 = px.imshow(
        heat_pivot,
        color_continuous_scale=["#f0fdf9", "#0d9488", "#7f1d1d"],
...
with p2:
    city_dept = (
        df[df.status == "completed"]
        .groupby(["city", "department"])["fee_charged"]
        .sum()
        .reset_index()
    )
    fig9 = px.sunburst(
        city_dept,
        path=["city", "department"],
        values="fee_charged",
        color="fee_charged",
        color_continuous_scale=["#ccfbf1", "#0d9488", "#134e4a"],
...
if predict_btn:
    st.markdown(
        "<div class='section-head'>🔮 No-Show Risk Prediction</div>",
        unsafe_allow_html=True,
    )
    try:
        ...
        if proba > 25:
            st.info(
                "💡 **Reminder recommended** — Send an SMS/WhatsApp reminder 24h and 2h before this appointment."
            )
